...
int lua_cpuid(lua_State *L) {
	unsigned long long rax = 0, rbx = 0, rcx = 0, rdx = 0;
...
int lua_writeBytes(lua_State *L) {
	if (lua_gettop(L) >= 2) {
		if (!lua_istable(L, 2)) {
			sendstring("INF: lua_writeBytes: Parameter 2 must be a table\n");
			return 0;
		}
...
lua_State *initializeLua(void) {
	if (LuaVM == NULL) {
		...
		if (LuaVM) {
			sendstringf("INF: LuaVM created at %p\n", LuaVM);
...
void enterLuaConsole(void) {
	...
	sendstringf("INF: Welcome to DBVM's Lua console. Enter your code here (Ctrl+X executes the code)\n");

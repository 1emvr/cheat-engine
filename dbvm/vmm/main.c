...
int cpu_familyID = 0;
int cpu_type = 0;
int cpu_ext_modelID = 0;
int cpu_ext_familyID = 0;
...
volatile int initializedCPUCount = 0; //assume the first (main) cpu managed to start up
...
void vmm_entry2(void) {
	...
	sendstringf("INF: Setting up cpunr=%d\n",cpunr);
	...
	displayline("INF: %d: New CPU CORE. CPUNR=%d APICID=%d (cpuinfo struct at : %p rsp=%x)\n",cpunr, cpuinfo->cpunr, cpuinfo->apicid, cpuinfo, getRSP());
	...
	sendstringf("INF: Starting VMX for cpu %d\n", cpunr);
	displayline("INF: CPU CORE %d: entering VMX mode\n",cpunr);

	startvmx(cpuinfo);

	nosendchar[getAPICID()] = 0;
	sendstringf("INF: Application cpu returned from startvmx\n\r");
...
void vmm_entry(void) {
  ...
  if (isAP) {
	  vmm_entry2();
	  nosendchar[getAPICID()] = 0;
	  sendstringf("INF: vmm_entry2 has PHAILED!!!!");
  ...
  sendstringf("INF: MAXPHYADDR=%d\n", MAXPHYADDR);
  sendstringf("INF: MAXPHYADDRMASK=%6\n", MAXPHYADDRMASK);
  sendstringf("INF: MAXPHYADDRMASKPB=%6\n", MAXPHYADDRMASKPB);
  //enableserial();

  sendstringf("INF: If you see this that means that the transition from unpaged to paged was a success\n\r");
  sendstringf("INF: loadedOS=%6\n",loadedOS);

  currentdisplayline = 7;
  displayline("BOOT CPU CORE initializing\n");

  displayline("INF: CR3=%6\n", getCR3());
  displayline("INF: pagedirlvl4=%6\n",(UINT64)pagedirlvl4);
  displayline("INF: &pagedirlvl4=%6\n",(UINT64)&pagedirlvl4);
  displayline("INF: vmmstart=%6 (this is virtual address 00400000)\n",(UINT64)vmmstart);

  //displayline("press any key to continue\n");
  //initialize
  sendstring("INF: Welcome to Dark Byte\'s Virtual Machine Manager\n\r");
  sendstringf("INF: pagedirlvl4=%6\n\r",(unsigned long long)pagedirlvl4);

  sendstring("INF: Initializing MM\n\r");
  InitializeMM((QWORD)pagedirlvl4 + 4096);
  sendstring("INF: Initialized MM\n\r");
  /*
   * POST INIT
   */
  cpucount = 1;
  cpuinfo = malloc2(sizeof(tcpuinfo) < 4096 ? 4096 : sizeof(tcpuinfo));
  sendstringf("INF: allocated cpuinfo at %6\n\r", cpuinfo);
  zeromemory(cpuinfo, sizeof(tcpuinfo));
  cpuinfo->active = 1;
  cpuinfo->cpunr = 0;
  cpuinfo->apicid = getAPICID();
  ...
  sendstringf("INF: initialized cpuinfo at %6\n\r", cpuinfo);
  ...
  sendstringf("INF: IA32_APIC_BASE=%6\n\r", IA32_APIC_BASE);
  ...
  displayline("INF: IA32_APIC_BASE=%6\n\r",IA32_APIC_BASE);
  sendstringf("INF: IA32_APIC_BASE=%6\n\r",IA32_APIC_BASE);
 ...
 // if (0)
  if (1) { //((d & (1<<28))>0) //this doesn't work in vmware, so find a different method
	  ...
	  unsigned int foundcpus = 0;
	  sendstring("INF: Multi processor supported\n\r");
	  sendstring("INF: Launching application cpu's\n");

	  //displaystring("Multi processor supported\n");
	  displayline("INF: Launching other cpu cores if present\n");
	  sendstring("INF: Starting other cpu's\n\r");
	  ...
	  if (loadedOS) {
		  sendstringf("INF: mapping loadedOS (%6)...\n", loadedOS);

		  POriginalState original = (POriginalState)mapPhysicalMemory(loadedOS, sizeof(OriginalState));
		  sendstringf("INF: Success. It has been mapped at virtual address %6\n", original);

		  entrypage = original->APEntryPage;
		  sendstringf("INF: original->cpucount=%d\n", original->cpucount);

		  if (original->cpucount > 1000) {
			  nosendchar[getAPICID()] = 0;
			  sendstringf("INF: More than 1000 cpu\'s are currently not supported\n");
			  ddDrawRectangle(0, DDVerticalResolution - 100, 100, 100, 0xff0000);
			  while (1);
		  }
		  ...
		  if (original->FrameBufferBase) {
			  ...
			  if (DDFrameBufferBase) {
				  ...
				  if (DDFrameBuffer == NULL) {
					  sendstring("ERR: Failure mapping memory");
				  } else {
	  ...
	  if (needtospawnApplicationProcessors) { //e.g UEFI boot with missing mpsupport
		  sendstringf("INF: needtospawnApplicationProcessors!=0\n");
#ifndef NOMP
		  ...
		  if (GDTsize > 192) {
			  sendstringf("INF: Update the AP boot GDT section to be bigger than 192 bytes");
		  }
		  ...
		  sendstringf("INF: vmmentrycount before launch=%d\n", vmmentrycount);
	  ...
	  displayline("INF: Wait done. Cpu's found : %d (expected %d)\n",vmmentrycount, foundcpus);
	  sendstringf("INF: vmmentrycount after launch=%d\n", vmmentrycount);
  ...
  if (GDT_BASE == NULL) {
	  nosendchar[getAPICID()] = 0;
	  sendstring("INF: Memory allocation failed\n");
	  ddDrawRectangle(0, DDVerticalResolution - 100, 100, 100, 0xff0000);
  ...
  sendstringf("INF: Allocated GDT_BASE %6\n", GDT_BASE);

  {
    void *GDTbase = (void*)getGDTbase();
    int GDTsize = getGDTsize();

    sendstringf("INF: getGDTbase=%p, getGDTsize=%d\n",GDTbase,GDTsize );
    copymem(GDT_BASE,GDTbase,GDTsize);
  }

  sendstringf("INF: Allocated and copied GDT to %x\n\r",(UINT64)GDT_BASE);
  ...
  sendstringf("INF: Allocated intvector at %6\n\r",(unsigned long long)intvector);

  setints();
  sendstring("INF: after setints()\n");

  i = 0;
  setDR0((QWORD)((volatile void*)&&BPTest));
  setDR6(0xffff0ff0);
  setDR7(getDR7() | (1 << 0));
  displayline("INF: Going to execute test breakpoint\n");
...
AfterBPTest:
  if (i == 0)
    sendstringf("INF: BPTest success.  dr6=%6\n", getDR6());
  else
    sendstring(":(\n");
  ...
  try {
    sendstring("INF: Trying memory access BP\n");
    ...
    setDR6(0xffffffff);
    sendstringf("INF: Before access DR6=%6\n",getDR6());
    asm volatile ("": : :"memory");
    debugtestvar = 0;
    debugtestvar = 890;
    asm volatile ("": : :"memory");

    sendstringf("INF: Memory access BP failure\n DR1=%6 DR7=%6", getDR1(), getDR7());

  } except {
    sendstringf("INF: Except block success.  dr6=%6\n", getDR6());
  }
  tryend;

  sendstringf("INF: Letting the first AP cpu go through\n");
  startNextCPU();

  fakeARD = malloc(4096);
  fakeARD[0].Type = 255;
  sendstringf("INF: Allocated fakeARD at %6\n",(unsigned long long)fakeARD);
  sendstringf("INF: That is physical address %6\n", VirtualToPhysical(fakeARD));

  if (!loadedOS) {
    sendstring("INF: Copying ARD from 80000 to ARD location");
  ...
  sendstring("INF: Calling initARDcount()\n");
  initARDcount();
  sendstring("INF: Calling sendARD()\n");
  sendARD();

  sendstring("INF: after sendARD()\n");
  ...
  sendstringf("INF: Physical address of ffpage=%6\n\r",(UINT64)VirtualToPhysical(ffpage));
  ...
  sendstringf("INF: Physical address of ffpagetable=%6\n\r",(UINT64)VirtualToPhysical((void *)ffpagetable));
  ...
  sendstringf("INF: Physical address of ffpagedir=%6\n\r",(UINT64)VirtualToPhysical((void *)ffpagedir));

  __asm("mov %cr3,%rax\n  mov %rax,%cr3\n");

  displayline("INF: emulated virtual memory has been configured\n");

  displayline("INF: Paging:\n");
  ...
  displayline("INF: Calling hascpuid()\n");
	if (hascpuid()) {
		char *t;
		a = 0; b=0; c=0; d=0;

		_cpuid(&a, &b, &c, &d);
		sendstringf("INF: Your comp supports cpuid! (%d , %x %x %x )\n\r", a, b, d, c);
		...
		if ((b == 0x68747541) && (d == 0x69746e65) && (c == 0x444d4163)) {
			...
			sendstring("INF: This is an AMD system. going to use the AMD virtualization tech\n\r");
		} else {
			isAMD = 0;
    ...
    //if (!(a & 0x80000000))
		...
		sendstring("INF: Your crappy comp doesn\'t support cpuid\n\r");
		displayline("INF: Your system does not support CPUID\n");
	}
	...
	sendstring("INF: setting up gdt entry at offset 0x64 as virtual8086 task\n\r");
	PGDT_ENTRY currentgdt = (PGDT_ENTRY)getGDTbase();


	sendstringf("INF: currentgdt is %x (limit=%x)\n\r", (UINT64)currentgdt, getGDTsize());
  ...
  //if (!loadedOS)
	{
		sendstringf("INF: Setting up 64-bit TS and TSS\n\r");

		TSS64 *temp = malloc(4096);

		sendstringf("INF: temp allocated at %x\n", temp);
		ownTSS = temp;
	...
	displayline("INF: Generating debug information\n\r");
	originalVMMcrc = generateCRC((void*)vmxloop, 0x2a000);

	displayline("INF: Virtual machine manager loaded\n");
#ifdef DEBUG
	displayline("INF: Entering menu system\n");
#else
	displayline("INF: Skipping menu system and autostarting VM\n");
#endif
...
void vmcalltest(void) {
	...
	try {
		...
		sendstringf("INF: dbvm is loaded. Version %x\n", dbvmversion);
	} except {
		sendstringf("INF: dbvm is not loaded\n");
	}
	tryend;
}

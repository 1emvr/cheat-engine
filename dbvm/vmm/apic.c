...
void apic_sendWaitInterrupt(BYTE apicid) {
	...
	if (sizeof(APIC_ICR)! = 8) {
		sendstringf("INF: sizeof(APIC_ICR) = %d\n",sizeof(APIC_ICR));
		while (1);
	}

	sendstringf("INF: apic_nmi(%d)\n",apicid);
	...
	sendstringf("INF: sending APIC command to wait to cpu with APICID %d", apicid);
	...
	if (readMSR(IA32_APICBASE_MSR) & (1 << 10)) {
		...
		SetPageToWriteThrough(apic);
		sendstringf("INF: Waiting till ready\n");
		...
		sendstringf("INF: Waiting till ready again\n");

...
void setints(void) {
	...
	if (intvector == NULL) {
		sendstring("INF: setints was called too early");
...
int cinthandler(unsigned long long *stack, int intnr) { //todo: move to it's own sourcefile
  PRFLAGS rflags;
  int errorcode = 0;
  UINT64 errorcodeValue = 0;
  int i = 0;
  int cpunr = 0;

  DWORD thisAPICID = getAPICID();
 // nosendchar[getAPICID()]=0;
  ...
  sendstring("\n------------------------------------------\n");
  sendstringf("       UH OH !!! EXCEPTION %d  (%x)        \n", intnr, intnr);
  sendstring("------------------------------------------\n");

  ddDrawRectangle(DDHorizontalResolution - 100, 0, 100, 100, _rdtsc());

  if (readMSR(IA32_FS_BASE_MSR) == 0) {
    sendstringf("INF: Invalid FS base during exception %d  CR2=%6!!\n",intnr, getCR2());

    if (intnr == 13) {
      UINT64 RIP = stack[17];
      errorcodeValue = stack[16];

      sendstringf("INF: RIP=%6\n", RIP);
      sendstringf("INF: ErrorCode=%x\n", errorcodeValue);
    }
  ...
  if ((intnr == 2) && (cpuinfo->WaitTillDone)) {
    sendstringf("INF: %d: NMI received while handling a vmexit\n", getcpunr());

    cpuinfo->WaitingTillDone = 1;
    while (cpuinfo->WaitTillDone) {
		_pause();
	}
  ...
  if (thisAPICID != cpuinfo->apicid) {
    ...
    sendstringf("INF: Interrupt %d. Invalid cpuinfo", intnr);
    while(1);
  }
#endif

 // sendstringf("interrupt fired : %d (%x)\n\r", intnr,intnr);
  sendstringf("INF: cpunr=%d (apicid=%d)\n\r", cpunr, thisAPICID);
  sendstringf("INF: intnr=%d\n\r", intnr);
  sendstringf("INF: rsp=%6\n\r", getRSP());
  sendstringf("INF: cr2=%6\n\r", getCR2());
  errorcode = 0;
  ...
  if (errorcode) {
    errorcodeValue = stack[16];
    sendstringf("INF: Interrupt has errorcode : %x (",errorcodeValue);
  ...
  sendstringf("INF: rip=%x\n\r",stack[16+errorcode]);
  sendstringf("INF: rflags=%x\n\r",stack[16+2+errorcode]);
  ...
  sendstringf("INF: Checking if it was an expected interrupt\n\r");
  ...
  if (cpuinfo->OnInterrupt.RIP) {
    ...
    sendstringf("INF: Yes, OnInterrupt is set to %x\n\r",cpuinfo->OnInterrupt);
    ...
    sendstringf("INF: changed rip(was %6 is now %6)\n\r", oldrip, stack[16 + errorcode]);
    sendstringf("INF: rflags upon return is %x\n\r", stack[16 + 2 + errorcode]);
    sendstring("INF: returning now\n");

    return errorcode;
  }

  sendstring("INF: not expected\n\r");
  ddDrawRectangle(0, DDVerticalResolution - 100, 100, 100, 0xff0000);
  ...
  sendstringf("INF: Trying to disassemble caller instruction\n\r");
  ...
  while (start < 30) {
	  distorm_decode(
			  stack[16 + errorcode] - 30 + start, 
			  (unsigned char*)(stack[16 + errorcode] - 30 + start), 
			  120, Decode64Bits, disassembled, 22, &used);
  ...
  autostart = 0;
  setDR7(originalDR7);
  sendstring("INF: End of interrupt\n\r");

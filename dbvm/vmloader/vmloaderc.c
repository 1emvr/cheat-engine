...
int _vmloader_main(void) {
	...
	if (!isAP)
	{
		...
		printstring("www.(redacted).org", 40-8, 3, 12, 1);

		currentdisplayline=7;

		displayline("INF: a20state = %d (should be 1)\n",a20state);
		displayline("INF: bootdrive = %2 (stored at %p)\n",bootdisk, &bootdisk);
		displayline("INF: SectorsPerTrack = %d\n",SectorsPerTrack);
		displayline("INF: NumberOfHeads = %d\n",NumberOfHeads);
		displayline("INF: NumberOfCylinders = %d\n",NumberOfCylinders);

		displayline("INF: &bootsector=%p\n", &bootsector);

		displayline("INF: testing diskread ability....");


		//  waitforkeypress();

		if (!readsector(0, bootsector)) {
			sendstringf("ERR: failed loading the bootsector.\n");
			displayline("ERR: failed loading the bootsector.\n");
			...
			displayline("INF: successfully loaded the bootsector\n");
		...
		zeromemory(bootsectorm, 1024);
		sendstringf("INF: VMM starts at sector %d\n\r",VMMlocation);
		sendstringf("INF: LOGO starts at sector %d\n\r",LOGOlocation);

		displayline("INF: VMM starts at sector %d\n",VMMlocation);
		...
		if (chosenregion == -1) {
			displayline("ERR: failure in picking a fitting region\n");
			halt();
		}
		...
		if (maxAvailableAddress==0) {
			sendstringf("ERR: not enough usable memory (at end)\n\r");
			displayline("ERR: not enough usable memory (at end)\n\r");
		} else {
			...
			sendstringf("INF: Max address = %6  (region %d)\n\r", maxAvailableAddress, chosenregion);
			...
			sendstringf("INF: adjusting memory map (done for physical memory access devices)\n\r");
			displayline("INF: adjusting memory map (done for physical memory access devices)\n\r");
			...
			if (p[chosenregion+1].Type == 2) //I hope so
			{
				...
				sendstringf("INF: adjusted reserved memory size as well\n\r");
				displayline("INF: adjusted reserved memory size as well\n\r");
			}
			else
			{
				//insert a new one
				...
				sendstringf("INF: inserted new region\n\r");
				displayline("INF: inserted new region\n\r");
			}

			//  waitforkeypress();
			//  waitforkeypress();

			sendstringf("INF: newmap = \n\r");
			displayline("INF: newmap = \n\r");
			...
			sendstringf("INF: going to zero 0x00400000 bytes at %p\n", start);
			displayline("INF: Going to zero 0x00400000 bytes at %p\n", start);
			//waitforkeypress();
			//waitforkeypress();
			...
			sendstringf("INF: after zeroing 0x00400000 bytes at %p\n", start);
			// waitforchar();
			...
			displayline("INF: reading %d bytes from disk into %8 ...",VMMSIZE, (unsigned int)start);

			displayline("INF: VMMlocation = %8\n",VMMlocation);
			displayline("INF: start = %8\n",start);
			displayline("INF: VMMSIZE = %8\n",VMMSIZE);
			...
			while (bytesread < VMMSIZE)
			{
				if (readsector(VMMlocation + (bytesread / 512), (void*) ((ULONG)start + bytesread))) {
					...
					displayline("ERR: read error after %d bytes\n",bytesread);
					while (1) {
						displayline("ERROR!\n");  // oh my god...
					}
			...
			sendstringf("INF: PageMapLevel4 = %8\n\r", PageMapLevel4);
			displayline("INF: PageMapLevel4 = %8\n\r", PageMapLevel4);
			...
			*(QWORD*) (&PageDir[3]) = (QWORD)(DWORD)pagetable3;
			...
			sendstringf("INF: PageDirPtr[0].PFN = %8\n\r", PageDirPtr[0].PFN);
			...
			sendstringf("INF: vmloader finished. Switching from 32-bit to 64-bit and entering VMM\n");
			displayline("INF: vmloader finished. Switching from 32-bit to 64-bit and entering VMM\n");
		...
		sendstringf("INF: this is a Application Processor\n\r");
		gotoVMM();
	}

	sendstringf("INF: I managed to get to the end of the function. YEEEH!!!\n\r");
	return 0;
};
